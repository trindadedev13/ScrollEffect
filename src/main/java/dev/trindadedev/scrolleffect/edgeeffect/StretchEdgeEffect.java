...
/**
 * This class performs the graphical effect used at the edges of scrollable widgets when the user
 * scrolls beyond the content bounds in 2D space.
 *
 * <p>EdgeEffect is stateful. Custom widgets using EdgeEffect should create an instance for each
 * edge that should show the effect, feed it input data using the methods {@link #onAbsorb(int)},
 * {@link #onPull(float)}, and {@link #onRelease()}, and draw the effect using {@link #draw(Canvas)}
 * in the widget's overridden {@link android.view.View#draw(Canvas)} method. If {@link
 * #isFinished()} returns false after drawing, the edge effect's animation is not yet complete and
 * the widget should schedule another drawing pass to continue the animation.
 *
 * <p>When drawing, widgets should draw their main content and child views first, usually by
 * invoking <code>super.draw(canvas)</code> from an overridden <code>draw</code> method. (This will
 * invoke onDraw and dispatch drawing to child views as needed.) The edge effect may then be drawn
 * on top of the view's content using the {@link #draw(Canvas)} method.
 */
public class StretchEdgeEffect extends EdgeEffectCompat {
  ...
  /**
   * The velocity threshold before the spring animation is considered settled. The idea here is that
   * velocity should be less than 0.1 pixel per second.
   */
  private static final double VELOCITY_THRESHOLD = 0.01;

  /**
   * The speed at which we should start linearly interpolating to the destination. When using a
   * spring, as it gets closer to the destination, the speed drops off exponentially. Instead of
   * landing very slowly, a better experience is achieved if the final destination is arrived at
   * quicker.
   */
  private static final float LINEAR_VELOCITY_TAKE_OVER = 200f;

  /**
   * The value threshold before the spring animation is considered close enough to the destination
   * to be settled. This should be around 0.01 pixel.
   */
  private static final double VALUE_THRESHOLD = 0.001;

  /**
   * The maximum distance at which we should start linearly interpolating to the destination. When
   * using a spring, as it gets closer to the destination, the speed drops off exponentially.
   * Instead of landing very slowly, a better experience is achieved if the final destination is
   * arrived at quicker.
   */
  ...
  /**
   * The variation of the velocity for the stretch effect when it meets the bound. if value is > 1,
   * it will accentuate the absorption of the movement.
   */
  ...
  public StretchEdgeEffect(
      Context context, Runnable invalidate, Runnable postInvalidateOnAnimation) {
    this(context);
  ...
  /**
   * Reports if this EdgeEffect's animation is finished. If this method returns false after a call
   * to {@link #draw(Canvas)} the host widget should schedule another drawing pass to continue the
   * animation.
   *
   ...
   * always {@link android.view.View#invalidate()} after this and draw the results accordingly.
   *
   * <p>Views using EdgeEffect should favor {@link #onPull(float, float)} when the displacement of
   * the pull point is known.
   *
   ...
   *     initiating the pull. In the case of touch this is the finger position. Values may be from
   *     0-1.
   */
  @Override
   ...
   * A view should call this when content is pulled away from an edge by the user. This will update
   * the state of the current visual effect and its associated animation. The host view should
   * always {@link android.view.View#invalidate()} after this and draw the results accordingly. This
   * works similarly to {@link #onPull(float, float)}, but returns the amount of <code>deltaDistance
   * </code> that has been consumed. If the {@link #getDistance()} is currently 0 and <code>
   * deltaDistance</code> is negative, this function will return 0 and the drawn value will remain
   * unchanged.
   *
   * <p>This method can be used to reverse the effect from a pull or absorb and partially consume
   * some of a motion:
   *
   ...
   *     initiating the pull. In the case of touch this is the finger position. Values may be from
   *     0-1.
   * @return The amount of <code>deltaDistance</code> that was consumed, a number between 0 and
  ...
  /**
   * Returns the pull distance needed to be released to remove the showing effect. It is determined
   * by the {@link #onPull(float, float)} <code>deltaDistance</code> and any animating values,
   * including from {@link #onAbsorb(int)} and {@link #onRelease()}.
   *
   * <p>This can be used in conjunction with {@link #onPullDistance(float, float)} to release the
   * currently showing effect.
   *
  ...
  public void applyStretch(
      Canvas canvas, @EdgeEffectPosition int position, int translationX, int translationY) {
    mTmpOut[0] = 0f;
   ...
   * Y=0 and extending to some factor < 1.f of height. The effect will only be visible on a hardware
   * canvas, e.g. {@link RenderNode#beginRecording()}.
   *
  ...
  /**
   * Return the maximum height that the edge effect will be drawn at given the original {@link
  ...
  private void updateSpring() {
    ...
    double sinCoeff =
        (1 / mDampedFreq) * (DAMPING_RATIO * NATURAL_FREQUENCY * mDistance * mHeight + mVelocity);
    double distance =
    ...
    double velocity =
        distance * (-NATURAL_FREQUENCY) * DAMPING_RATIO
            + Math.pow(Math.E, -DAMPING_RATIO * NATURAL_FREQUENCY * deltaT)
  ...
  /**
   * @return true if the spring used for calculating the stretch animation is considered at rest or
   *     false if it is still animating.
   */
