 ...
 *
 * @param density to be taken into consideration during computations; Cupertino formulas use DPs,
 *   and scroll machinery uses raw values.
 * @param applyClip Some consumers of overscroll effect apply clip by themselves and some don't,
 *   thus this flag is needed to update our modifier chain and make the clipping correct in every
 *   case while avoiding redundancy
 */
...
class CupertinoOverscrollEffect(
  ...
  private fun applyToScroll(
    ...
    // Calculate how much delta is available after being consumed by scrolling inside overscroll
    // area
  ...
  override fun applyToScroll(
    ...
    return source.toCupertinoScrollSource()?.let { applyToScroll(delta, it, performScroll) }
      ?: performScroll(delta)
  }
  ...
  private suspend fun playSpringAnimation(
    ...
    val spec =
      when (reason) {
        CupertinoSpringAnimationReason.FLING_FROM_OVERSCROLL -> {
          spring(stiffness = 300f, visibilityThreshold = visibilityThreshold)
        }

        CupertinoSpringAnimationReason.POSSIBLE_SPRING_IN_THE_END -> {
          spring(stiffness = 120f, visibilityThreshold = visibilityThreshold)
        }
      }
    ...
    springAnimationScope?.run {
      AnimationState(Float.VectorConverter, initialValue / density, initialVelocity / density)
  ...
  private fun Offset.reverseHorizontalIfNeeded(): Offset =
    Offset(if (reverseHorizontal) -x else x, y)
...
/**
 * A class that represents the animation specification for a scroll decay animation using iOS-style
 * decay behavior.
 *
 * @property decelerationRate The rate at which the velocity decelerates over time. Default value is
 *   equal to one used by default UIScrollView behavior.
 */
class CupertinoScrollDecayAnimationSpec(
  //    private val decelerationRate: Float = UIScrollViewDecelerationRateNormal.toFloat()
  private val decelerationRate: Float = 0.998f // TODO поменять близко к айоси
...
internal fun convertNanosToSeconds(nanos: Long): Double = nanos.toDouble() / SecondsToNanos
...
internal interface ScrollableDefaultFlingBehavior : FlingBehavior {
  /**
   * Update the internal parameters of FlingBehavior in accordance with the new [Density] value.
   *
